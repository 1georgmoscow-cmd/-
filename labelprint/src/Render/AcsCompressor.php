...
final class AcsCompressor
{
    ...
    public static function compress(string $data, int $bytesPerRow, int $height): string
    {
        ...
        for ($y = 0; $y < $height; $y++) {
            ...
            // ':' повторяет предыдущую строку. В самом начале данных он недопустим:
            // ведущее двоеточие — это признак конверта :B64:/:Z64:, и принтер
            // попытается разобрать строку как испорченный ZB64.
            if ($row === $previousRow && $out !== '') {
                $out .= ':';
                continue;
            }
    ...
    public static function compressRow(string $hex): string
    {
        ...
        while ($i < $len) {
            ...
            if ($i + $run === $len && ($char === '0' || $char === 'F')) {
                // Требование мануала (Advanced Techniques, «Reducing Download Time of
                // Graphic Images»): перед ',' или '!' должно остаться ЧЁТНОЕ число
                // литеральных полубайт. Если хвост нечётной длины, один символ
                // выводится явно, и только остаток сворачивается. Иначе граница
                // байта уезжает и принтер разбирает строку не с того полубайта.
                if ($run % 2 === 1) {
                    $out .= $char;
                    $run--;
                }
